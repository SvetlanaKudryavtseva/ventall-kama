	...
	for($index = 0; $index < $cnt; ++$index){
		...
		if ($NextSectionID) {
			...
			if ($bShowCatalogSubsections) {
				$arSubSections = CAllCorp3::getChainNeighbors($NextSectionID, $arItem['LINK']);
			}
		}
		if($index){
		...
		if($arItem["LINK"] <> "" && $arItem['LINK'] != GetPagePath() && $arItem['LINK']."index.php" != GetPagePath() || $arSubSections){
			...
			if($arSubSections){
				...
				$strReturn .=($arSubSections ? '<span itemprop="name" class="breadcrumbs__item-name font_13">'.$title.'</span><span class="breadcrumbs__arrow-down '.(!$bLast ? 'colored_theme_hover_bg-el-svg' : '').'">'.CAllCorp3::showIconSvg("arrow", SITE_TEMPLATE_PATH."/images/svg/trianglearrow_down.svg").'</span>' : '<span>'.$title.'</span>');
				...
				$strReturn .= '<div class="breadcrumbs__dropdown-wrapper"><div class="breadcrumbs__dropdown rounded3">';
					foreach($arSubSections as $arSubSection){
						if ($arSubSection["LINK"] !== $arItem["LINK"]) {
							$strReturn .= '<a class="breadcrumbs__dropdown-item font_13" href="'.$arSubSection["LINK"].'">'.$arSubSection["NAME"].'</a>';
						}
					}
				$strReturn .= '</div></div>';
			}
